...
def play_alarm_sound(duration=3):
    """
    Play a doorbell-style ding-dong alarm sound for the specified duration.
    Uses Glass.aiff (high ding) + Pop.aiff (low dong) to simulate a doorbell.
    ...
    try:
        if sys.platform == "darwin":  # macOS
            import time as time_mod
            ding = MACOS_SOUNDS.get("bell", "/System/Library/Sounds/Glass.aiff")   # high note
            dong = MACOS_SOUNDS.get("pop",  "/System/Library/Sounds/Pop.aiff")     # low note
            end_time = time_mod.time() + max(0, int(duration))
            while time_mod.time() < end_time:
                os.system(f"afplay {ding} > /dev/null 2>&1")   # DING
                time_mod.sleep(0.45)
                os.system(f"afplay {dong} > /dev/null 2>&1")   # DONG
                time_mod.sleep(0.80)                            # pause before next ring
        elif sys.platform == "win32":  # Windows
            import winsound

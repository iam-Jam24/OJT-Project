...
### Syntax:
```bash
scheduler-cli add --name "job_name" --time "YYYY-MM-DDTHH:MM:SS" --freq once
```

### Examples:
```bash
# Run job on December 5, 2025 at 3:30 PM
scheduler-cli add --name "backup_full" --time "2025-12-05T15:30:00" --freq once

# Run job on December 1, 2025 at 9:00 AM
scheduler-cli add --name "morning_report" --time "2025-12-01T09:00:00" --freq once

# Run job on December 31, 2025 at 11:59 PM
scheduler-cli add --name "year_end" --time "2025-12-31T23:59:00" --freq once
```
...
### Syntax:
```bash
scheduler-cli add --name "job_name" --time "YYYY-MM-DDTHH:MM:SS" --freq daily
```

### Examples:
```bash
# Run job every day at 2:00 AM (starting Dec 2, 2025)
scheduler-cli add --name "daily_cleanup" --time "2025-12-02T02:00:00" --freq daily

# Run job every day at 6:00 PM (starting Dec 1, 2025)
scheduler-cli add --name "daily_sync" --time "2025-12-01T18:00:00" --freq daily

# Run job every day at 12:00 PM (noon)
scheduler-cli add --name "lunch_reminder" --time "2025-12-01T12:00:00" --freq daily
```
...
### Syntax:
```bash
scheduler-cli add --name "job_name" --time "YYYY-MM-DDTHH:MM:SS" --freq weekly
```

### Examples:
```bash
# Run job every Monday at 9:00 AM
scheduler-cli add --name "weekly_report" --time "2025-12-01T09:00:00" --freq weekly

# Run job every Friday at 5:00 PM
scheduler-cli add --name "weekly_meeting" --time "2025-12-05T17:00:00" --freq weekly

# Run job every Sunday at 8:00 PM
scheduler-cli add --name "weekly_backup" --time "2025-12-07T20:00:00" --freq weekly
```
...
### Syntax:
```bash
scheduler-cli add --name "job_name" --time "YYYY-MM-DDTHH:MM:SS" --freq hourly
```

### Examples:
```bash
# Run job every hour at :00 (top of the hour)
scheduler-cli add --name "health_check" --time "2025-12-01T00:00:00" --freq hourly

# Run job every hour at :30
scheduler-cli add --name "data_sync" --time "2025-12-01T12:30:00" --freq hourly

# Run job every hour at :15
scheduler-cli add --name "status_update" --time "2025-12-01T10:15:00" --freq hourly
```
...
### Syntax:
```bash
scheduler-cli add --name "job_name" --freq interval --seconds NUM
```

### Examples:
```bash
# Run job every 30 seconds
scheduler-cli add --name "monitor" --freq interval --seconds 30

# Run job every 5 minutes (300 seconds)
scheduler-cli add --name "quick_check" --freq interval --seconds 300

# Run job every 1 minute (60 seconds)
scheduler-cli add --name "heartbeat" --freq interval --seconds 60

# Run job every 1 hour (3600 seconds)
scheduler-cli add --name "hourly_task" --freq interval --seconds 3600
```
...
scheduler-cli add --name "one_time_backup" --time "2025-12-15T15:00:00" --freq once
...
scheduler-cli add --name "hourly_check" --time "2025-12-01T00:00:00" --freq hourly

# Every 5 minutes continuously
scheduler-cli add --name "continuous_monitor" --freq interval --seconds 300
```
...
```bash
# ❌ WRONG
scheduler-cli add --name "bad_time" --time "12/25/2025 3:30 PM" --freq daily

# ✅ CORRECT
scheduler-cli add --name "good_time" --time "2025-12-25T15:30:00" --freq daily
```

### Missing time for non-interval jobs:
```bash
# ❌ WRONG - missing --time
scheduler-cli add --name "task" --freq daily

# ✅ CORRECT
scheduler-cli add --name "task" --time "2025-12-02T09:00:00" --freq daily
```

### Missing seconds for interval jobs:
```bash
# ❌ WRONG - missing --seconds
scheduler-cli add --name "task" --freq interval

# ✅ CORRECT
scheduler-cli add --name "task" --freq interval --seconds 300
```

...
alarm-cmd 5s "break time"

# 3 minutes
alarm-cmd 3m

# 3 minutes with message
alarm-cmd 3m "take medicine"
...
alarm-cmd 2:30pm "meeting reminder"

# Verbose format
alarm-cmd "in 10 minutes" "check the oven"
```
...
alarm-cmd 7am "take vitamins"

# Alarm in 10 minutes to get ready
alarm-cmd 10m "get ready"
```

### Work Reminders
```bash
# 30-minute break reminder
alarm-cmd 30m "time for a break"

# 2 PM meeting reminder
alarm-cmd 2pm "team standup meeting"

# 1-hour quick task timer
alarm-cmd 1h "review emails"
```
...
# Test with message
alarm-cmd 3s "test notification"
```
...
Add a shorter alias to your `~/.zshrc` or `~/.bashrc`:

```bash
alias alarm='alarm-cmd'
```
...
Install the package once and the `alarm-cmd` command becomes available globally:
